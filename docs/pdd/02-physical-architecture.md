...
## 2.8 Feed/Friends Physical Addendum

The Feed adds no custom server or direct friend-to-Storage path. The authenticated Flutter client uses Firebase Authentication and Firestore for its own/current-friend post streams, with one authorUid equality query per author. Trusted reciprocal relationships are users/{uid}/friends/{friendUid}; directional blocks are users/{uid}/blockedUsers/{blockedUid}, and either direction revokes the next online access. Clients cannot create either relationship document.

An owner alone accesses staging at feed-thumbnail-staging/{uid}/{activityId}/{uploadId}.png. Cloud Functions promote compliant exact Running History bytes to immutable server-owned feed-thumbnails/{uid}/{activityId}/route-preview.png, recording final path, generation, and SHA-256 in an active feedPosts/{activityId} snapshot. The bytes are an 88-logical-pixel privacy-masked PNG with DPR capped at 3, 12-logical-pixel start/end masks, no metadata, and a 1 MiB maximum. Feed metadata and thumbnail bytes exclude raw GPS, route arrays, coordinates, addresses, and private profile data.

publishActivityToFeed is available only after explicit **Post to Feed** confirmation for an owned validated activity; completeRun never posts. readFeedThumbnail(postId) checks active post state, reporter hidden state, friendship, both blocks, and exact object binding before returning bounded PNG bytes. It never returns a signed URL and friends never directly read Storage. reportFeedPost, deleteFeedPost, retry-safe count triggers, and trusted source-activity cleanup remain Cloud Function responsibilities. Cached offline Feed data is read-only and all mutations are disabled. This physical extension neither changes subscriptionStatus/userRole boundaries nor creates XP, rank, leaderboard, or Premium competitive advantage.
