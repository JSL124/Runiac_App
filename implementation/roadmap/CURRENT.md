...
- Current active capsule: `implementation/roadmap/capsules/goal-plan-detail-header-timeline-alignment.md`
- Most recent completed capsule: `expert-plan-detail-static-snapshot-shell` implemented, pushed at `bd5c4b2c118bf0ce3776747418a40aa0d8e14007 feat(you): add expert plan detail preview`, and verified by hosted Governance CI #63 PASS for run ID `27151963998`.
...
- Current active milestone: `goal-plan-detail-header-timeline-alignment` is selected for local-only static Flutter UI refinement covering Goal Plan Detail header unification, accent strip placement, timeline marker alignment, horizontal week dividers, visual-only progress states, initially-collapsed weekly dropdowns, static Monday-to-Sunday daily plan rows, sample preview-only run/rest day mapping, and current-week full-row highlight. Phase 02 remains unselected, and implementation may begin only after this CURRENT.md selection is confirmed.
...
2. Active capsule document: `implementation/roadmap/capsules/goal-plan-detail-header-timeline-alignment.md`
...
Next gate is local-only implementation of the selected `goal-plan-detail-header-timeline-alignment` capsule. The selected scope includes refined static week-list behavior only: visual-only progress semantics, no visible `Completed`, `Current`, or `Upcoming` row labels, preview-only Monday-to-Sunday rows, and no persistence, onboarding state mutation, backend-owned progress state, or trusted personalization behavior. Implementation must stop at Ready for commit and must not be staged, committed, pushed, or expanded without separate explicit instruction.

## Operational TODO / Active Capsule

Active capsule: `goal-plan-detail-header-timeline-alignment`.
...
3. Do not load future phase documents or route a new capsule unless explicitly requested.
4. Treat the Goal Plan Detail, Weekly Workout Detail, and Expert Plan List capsules as closed unless a future explicit task reopens one of them.
...
8. Do not select or implement a next capsule, Phase 02 work, Firebase/backend/GPS/native/dependency work, or any new product scope without separate explicit approval.
9. For `goal-plan-detail-header-timeline-alignment`, static sample daily rows may represent a sample `10K Goal Plan` onboarding mapping only; they must not create enrollment, persistence, onboarding mutation, trusted current-week calculation, or backend-owned plan progress behavior.
