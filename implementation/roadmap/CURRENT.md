...
- Current active capsule: none selected after local completion and roadmap reconciliation of `implementation/roadmap/capsules/run-plan-objective-bottom-sheet.md`, `implementation/roadmap/capsules/backend-owned-streak-persistence.md`, `implementation/roadmap/capsules/run-duration-fields.md`, and `implementation/roadmap/capsules/running-activity-history-user-link.md`. The Plan/Run work now covers generated planned-workout launch/active objective display, duration-based generated workouts, rest-day launch context, notification/system re-entry preservation, backend-owned plan progress hydration, backend-owned streak persistence, active/elapsed/paused duration fields, and owner-scoped Activity History reads from `runSummaries`. It did not implement personalized adaptive estimate learning, Firebase deploy, Firestore rules/index deploy, `firebase init`, `flutterfire configure`, route trace upload, GPS sample persistence, shared route generation, background tracking, XP/level/rank/leaderboard formulas, premium entitlement logic, expert-plan publication logic, or Phase 02.
...
- Most recent locally closed implementation milestone: Plan/Run roadmap reconciliation closed stale local capsule states for backend-owned streak persistence, run duration fields, owner-linked Activity History, and Plan Feature Phase 1+2 progress hydration after source/test inspection on 2026-07-07 Asia/Singapore.
- Current status: Phase 01 governance CI closed; Artifact Inventory Schema persistence completed; Repository Workflow Record capsule closed; Flutter scaffold baseline present at `implementation/mobile/runiac_app/`; multiple static Flutter UI capsules are closed through Run Hold-to-End static interaction at `027c960`; Run Tracking backend/repository/emulator milestones M2, M3, and M5 are complete locally through `ff345e3`; M4-A/M4-B foreground run tracking milestones are complete locally through `f9fe044`; M4-C1 Local Map Follow Runner UI is complete locally at `b5124dd feat(run): add local map follow UI`; M4-C2 Mapbox Run Map Surface is complete locally at `4ac294c feat(run): add Mapbox run map surface`; M4-F GPS Quality Diagnostics/startup readiness UI is complete locally through `832761d fix(run): polish GPS startup UI` after blocker-fix validation and physical iPhone manual QA passed. Backend-owned streak persistence, run duration field split, owner-scoped Activity History, and Plan Feature Phase 1+2 progress hydration are locally complete and reconciled from existing source/test evidence. Production Firebase Auth/mobile config is connected for `runiac-fypp` as of `478898c0 feat(auth): connect production firebase auth`; emulator Auth/Functions behavior remains guarded by `RUNIAC_FIREBASE_EMULATOR=true`.
- Current state: Scaffold-baseline governance state with the Flutter mobile UI split into a feature-first-lite source structure, a guarded Run completion backend path, bounded production Firebase Auth/mobile config, a local Run Tracking engine, a foreground GPS permission/provider path, a Mapbox-backed run map surface gated by `MAPBOX_PUBLIC_ACCESS_TOKEN`, and local scalar GPS quality diagnostics for Run Tracking. Phase 02 remains unselected. The Run completion flow now has a repository seam, async hardening, emulator-only FlutterFire bootstrap, anonymous Auth emulator sign-in, and a `completeRun` callable adapter for `asia-southeast1` when `RUNIAC_FIREBASE_EMULATOR=true`; outside emulator mode, Auth bootstrap uses `FirebaseRuniacAuthRepository`. Firestore SDK/bootstrap exists, generated plans can be persisted under `generatedPlans/{uid}`, backend-owned plan progress is read from `planProgress/{uid}`, owner Activity History reads `runSummaries`, and backend-owned streak state is written by `completeRun` only after run validation. Missing-token placeholder fallback remains available, Android no-token fallback QA passed, token-backed Mapbox demo readiness is pending real demo-token QA, and the Mapbox network/telemetry boundary is documented. Raw route/GPS samples remain local-only, M4-F diagnostics remain scalar/local-only, M4-F physical iPhone QA passed, route trace backend upload is not done yet, shared route auto-generation is not done yet, GPS trace persistence is not done yet, background tracking is not done yet, personalized adaptive estimate learning is not done yet, Auto Pause and Moving Time remain future M4-G scope, real XP/level/rank/leaderboard formulas are not done yet, and leaderboard aggregation is not done yet.
...
- Current active milestone: none selected. Latest reconciled local Run Tracking milestones are M4-A Local Run Tracking Engine complete at `c3ee282 fix(run): harden local tracking engine`, M4-B Foreground GPS Permission + Provider complete at `adad6f4 feat(run): add foreground GPS provider`, M4-B permission guidance complete at `f9fe044 fix(run): improve foreground location permission guidance`, M4-C1 Local Map Follow Runner UI complete at `b5124dd feat(run): add local map follow UI`, M4-C2 Mapbox Run Map Surface complete at `4ac294c feat(run): add Mapbox run map surface`, M4-F GPS Quality Diagnostics/startup readiness UI complete through `07dd96e fix(ios): raise deployment target to 15` and `832761d fix(run): polish GPS startup UI`, plus reconciled local completion of backend-owned streak persistence, run duration fields, owner-linked Activity History, and Plan Feature Phase 1+2 progress hydration. Next recommended work is roadmap reconciliation commit/push decision, token-backed Mapbox QA with a real demo public token, personalized adaptive estimate learning, or a separately selected roadmap capsule. Phase 02 remains unselected.
...
- Still not implemented: route trace backend upload, GPS trace persistence, shared route auto-generation, background tracking, production Firestore deploy, personalized adaptive estimate learning, XP/level/rank formulas, and leaderboard aggregation.
...
Next gate is explicit token-backed Mapbox QA with a real demo public token, explicit personalized adaptive estimate learning capsule selection, explicit next capsule selection, or push/remote verification decision for this roadmap reconciliation. M4-C2 Mapbox Run Map Surface is complete locally at `4ac294c`, but token-backed demo readiness remains pending real-token QA. Mapbox token-backed QA must keep the token runtime-only via `--dart-define=MAPBOX_PUBLIC_ACCESS_TOKEN=...`; must not store the token in source, `.env`, README, plist, Android resources, or git; and must not stage SwiftPM `Package.resolved` artifacts. Required Mapbox QA evidence: token-backed Android emulator QA, token-backed physical iPhone QA, map render, route polyline render, camera/recenter behavior, attribution/logo visibility, startup/default camera behavior before GPS, no token leakage, and no `Package.resolved` staging. No Phase 02 selection, Firestore rules/index deploy, `flutterfire configure`, build, init, deploy, route trace backend upload, GPS trace persistence, shared route auto-generation, background tracking, personalized adaptive estimate learning, real XP/level/rank/leaderboard formulas, leaderboard aggregation, or additional Flutter/backend implementation outside a separately selected capsule is authorized without separate explicit approval.

## Operational TODO / Active Capsule

Active capsule: none selected after local completion and roadmap reconciliation of `implementation/roadmap/capsules/run-plan-objective-bottom-sheet.md`, `implementation/roadmap/capsules/backend-owned-streak-persistence.md`, `implementation/roadmap/capsules/run-duration-fields.md`, and `implementation/roadmap/capsules/running-activity-history-user-link.md`. The previous `implementation/roadmap/capsules/profile-persistence-rules-contract.md` capsule has been superseded locally by the profile edit/onboarding work. The previous `implementation/roadmap/capsules/firestore-base-bootstrap-seam.md` capsule is closed after the Firestore SDK/bootstrap seam sequence. The previously active `implementation/roadmap/capsules/complete-run-cloud-functions-emulator-skeleton.md` capsule is closed after the broader Run Tracking M2/M3/M5 sequence.
