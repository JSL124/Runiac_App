...
- Most recent locally closed capsule: `flutter-static-ui-demo-config-extraction` closed at `bdc1a47 refactor(mobile): extract static ui demo data`; roadmap reconciliation for this closure is pending commit.
- Current status: Phase 01 governance CI closed; Artifact Inventory Schema persistence completed; Repository Workflow Record capsule closed; Flutter scaffold baseline present at `implementation/mobile/runiac_app/`; multiple static Flutter UI capsules are closed through Run Hold-to-End static interaction at `027c960`; Run Tracking backend/repository/emulator milestones M2, M3, and M5 are complete locally through `ff345e3`; M4-A/M4-B foreground run tracking milestones are complete locally through `f9fe044`; M4-C1 Local Map Follow Runner UI is complete locally at `b5124dd feat(run): add local map follow UI`. FlutterFire is allowed only for emulator-only `completeRun` wiring guarded by `RUNIAC_FIREBASE_EMULATOR=true`; production Firebase remains forbidden.
- Current state: Scaffold-baseline governance state with the Flutter mobile UI split into a feature-first-lite source structure, a narrow emulator-only Run completion backend path, a local Run Tracking engine, a foreground GPS permission/provider path, and a local-only map follow-runner UI. Phase 02 remains unselected. The Run completion flow now has a repository seam, async hardening, emulator-only FlutterFire bootstrap, anonymous Auth emulator sign-in, and a `completeRun` callable adapter for `asia-southeast1` when `RUNIAC_FIREBASE_EMULATOR=true`; the static repository remains the default fallback when that flag is absent or false. Local runner marker, polyline, follow mode, and recenter UI exist without a map SDK. Raw route/GPS samples remain local-only, `completeRun` remains summary-only, M4-C2 actual map SDK integration is not implemented, route trace backend upload is not done yet, shared route auto-generation is not done yet, background tracking is not done yet, real XP/streak/level formula is not done yet, and leaderboard aggregation is not done yet.
...
- Current active milestone: none selected. Latest reconciled local Run Tracking milestones are M4-A Local Run Tracking Engine complete at `c3ee282 fix(run): harden local tracking engine`, M4-B Foreground GPS Permission + Provider complete at `adad6f4 feat(run): add foreground GPS provider`, M4-B permission guidance complete at `f9fe044 fix(run): improve foreground location permission guidance` after manual/emulator QA passed for the permission guidance path, and M4-C1 Local Map Follow Runner UI complete at `b5124dd feat(run): add local map follow UI`. Next recommended work is M4-C2 actual map SDK integration plan/approval only. Phase 02 remains unselected.

Run Tracking M4-A/M4-B/M4-C1 reconciliation:
...
- Still not implemented: M4-C2 actual map SDK integration, route trace backend upload, shared route auto-generation, background tracking, production Firebase, XP/streak/level formula, and leaderboard aggregation.
- M4-C2 requires separate plan/approval for dependency choice, API key/provider setup, native configuration, tile/network behavior, and privacy decisions.
- Next recommended work: M4-C2 actual map SDK integration plan/approval only.
...
Next gate is explicit next capsule selection or push/remote verification decision for the local implementation commits recorded before this roadmap reconciliation. Recommended next work is M4-C2 actual map SDK integration plan/approval only. No Phase 02 selection, production Firebase setup, `flutterfire configure`, dependency installation, build, init, deploy, native work, map SDK dependency, API key/provider setup, tile/network integration, route trace backend upload, shared route auto-generation, background tracking, real XP/streak/level formula, leaderboard aggregation, or additional Flutter implementation is authorized without separate explicit approval.
