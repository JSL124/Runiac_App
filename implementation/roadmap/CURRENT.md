...
- Current active capsule: `implementation/roadmap/capsules/backend-owned-streak-persistence.md`. This implementation-approved capsule persists official running consistency streak state through the trusted `completeRun` Cloud Functions path after run validation, while keeping Flutter as a read-only display boundary for official streak values. It approves only backend-owned streak persistence and display-boundary work; it does not approve Firestore rules/index deploy, Cloud Functions production deploy, `firebase init`, `flutterfire configure`, secrets/config, route trace upload, GPS sample persistence, XP/level/rank/leaderboard formula, leaderboard aggregation, premium entitlement logic, expert-plan publication logic, Phase 02, backend-owned client mutation outside the trusted backend path, or SwiftPM staging. The previous `implementation/roadmap/capsules/run-duration-fields.md` and `implementation/roadmap/capsules/running-activity-history-user-link.md` capsules remain in-progress local context and are temporarily paused while this narrower backend-owned streak persistence fix is routed.
...
Active capsule: `implementation/roadmap/capsules/backend-owned-streak-persistence.md`. The previous `implementation/roadmap/capsules/run-duration-fields.md` and `implementation/roadmap/capsules/running-activity-history-user-link.md` capsules remain in-progress local context and are temporarily paused for this narrower backend-owned streak persistence fix. The previous `implementation/roadmap/capsules/profile-persistence-rules-contract.md` capsule has been superseded locally by the profile edit/onboarding work. The previous `implementation/roadmap/capsules/firestore-base-bootstrap-seam.md` capsule is closed after the Firestore SDK/bootstrap seam sequence. The previously active `implementation/roadmap/capsules/complete-run-cloud-functions-emulator-skeleton.md` capsule is closed after the broader Run Tracking M2/M3/M5 sequence.
