...
- Current active capsule: `implementation/roadmap/capsules/complete-run-cloud-functions-emulator-skeleton.md`. This is retained as the Run Tracking backend/repository/emulator capsule-of-record for governance allowlisting, but its implementation state is closed/reconciled through M5: M2 `completeRun` emulator backend skeleton is complete at `4fcbf96 feat(functions): add complete run emulator skeleton`; M3 Run Repository Integration is complete at `a45b007 feat(run): add repository completion integration`; M3 async completion hardening is complete at `68bebf6 fix(run): harden async completion flow`; M5 FlutterFire emulator-only wiring is complete at `ff345e3 feat(run): wire complete run emulator repository`. No new implementation work is selected by this retained routing marker.
...
- Still not implemented: route trace backend upload, GPS trace persistence, shared route auto-generation, background tracking, production Firebase/deploy, XP/streak/level formula, and leaderboard aggregation.
...
10. Do not select or implement another capsule, Phase 02 work, further production Firebase/Auth/Firestore setup, FlutterFire production config expansion, GPS/native work, dependency work, or any new product scope without separate explicit approval.
