...
- Current active capsule: `implementation/roadmap/capsules/run-duration-fields.md`. This implementation-approved capsule splits completed-run duration into `durationSeconds`, `activeDurationSeconds`, `elapsedWallSeconds`, and `pausedDurationSeconds` so paused/rested completed activities can sync through the trusted `completeRun` path. It does not approve Firestore rules/index deploy, Cloud Functions production deploy, `firebase init`, `flutterfire configure`, secrets/config, route trace upload, GPS sample persistence, XP/streak/level/rank/leaderboard formula, leaderboard aggregation, premium entitlement logic, expert-plan publication logic, Phase 02, backend-owned client mutation, or SwiftPM staging. The previous `implementation/roadmap/capsules/running-activity-history-user-link.md` remains an in-progress local activity-history capsule that is temporarily paused while this narrower duration-field production sync fix is routed.
...
Active capsule: `implementation/roadmap/capsules/run-duration-fields.md`. The previous `implementation/roadmap/capsules/running-activity-history-user-link.md` capsule remains in-progress local context and is temporarily paused for this narrower completed-activity sync fix. The previous `implementation/roadmap/capsules/profile-persistence-rules-contract.md` capsule has been superseded locally by the profile edit/onboarding work. The previous `implementation/roadmap/capsules/firestore-base-bootstrap-seam.md` capsule is closed after the Firestore SDK/bootstrap seam sequence. The previously active `implementation/roadmap/capsules/complete-run-cloud-functions-emulator-skeleton.md` capsule is closed after the broader Run Tracking M2/M3/M5 sequence.
