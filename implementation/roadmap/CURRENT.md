...
- Current active capsule in this isolated worktree: `implementation/roadmap/capsules/adaptive-character-guidance.md`. It is an explicitly approved Backend Guarded Lane implementation capsule for emulator-only adaptive character guidance. This isolated selection does not close, supersede, mutate, or authorize production work for the unrelated `leaderboard-jurong-east-seed-verification` capsule on the main worktree, which remains Active / Production confirmation pending.
...
- Current status: Phase 01 governance CI remains closed and existing Run/Plan milestones remain reconciled. In this clean isolated worktree, adaptive character guidance is routed for emulator-only implementation with production deployment blocked pending approved privacy disclosure/consent evidence; no production deploy, secret, dependency change, commit, or real provider call is authorized. The main worktree's production-pending Leaderboard status remains unchanged.
- Current state: Adaptive character guidance may use a server-owned, authenticated aggregate-evidence/cache path and a Flutter display/cycle integration only through its Backend Guarded Lane capsule. Weekly Leaderboard, GPS/route persistence, background tracking, Auto Pause, Moving Time, and all unrelated Leaderboard work remain outside this worktree's scope.
- Current governance decision record: ADR-003 classifies the selected character work as Backend Guarded Lane. Its required capsule chain is `A0 -> A9 -> A5 -> A11 -> A10 -> A13 -> A6 -> A12 -> A8`; the ADR itself does not authorize production deployment, secrets, dependencies, or commits.
- Current active milestone in this isolated worktree: `adaptive-character-guidance`. Phase 02 remains unselected.
...
Next gate in this isolated worktree is Todo 2 of `adaptive-character-guidance`: define the deterministic trusted coaching evidence and typed server contracts with emulator-first TDD. Production deployment remains blocked until privacy disclosure/consent evidence is recorded and separately approved; App Check remains an open production gate. The Leaderboard's main-worktree next gate is unchanged: fresh read-only inventory followed by exact user confirmation before any production seed/refresh/read-back or cleanup. No Phase 02 selection, `flutterfire configure`, GPS/route persistence, background tracking, or unrelated Flutter/backend implementation is authorized.

## Operational TODO / Active Capsule

Active capsule in this isolated worktree: `implementation/roadmap/capsules/adaptive-character-guidance.md`. It is the sole selected character-agent implementation scope and is limited to emulator-only work. `implementation/roadmap/capsules/leaderboard-jurong-east-seed-verification.md` remains active and production-pending on the main worktree; it is not closed, mutated, or selected here. `implementation/roadmap/capsules/home-stage-map-density-polish.md` remains closed at `74f9c219`; Weekly Leaderboard and unrelated Phase 02 work remain excluded.
...
9. For the active adaptive-character-guidance capsule only, use emulator-only synthetic non-identifying fixtures and server-owned aggregate evidence/cache behavior within its exact scope. Production deploy, production connections, real provider calls, secrets, dependency changes, and App Check claims remain blocked pending their separate approvals.
10. Do not select or implement another capsule, Phase 02 work, feature-level Firestore reads/writes outside the active capsule, FlutterFire production config expansion beyond the existing Firebase config, GPS/native work, dependency work, production Leaderboard mutation, or any new product scope without separate explicit approval.
