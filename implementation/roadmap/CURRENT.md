...
- Current active capsule: none. Next work requires explicit future capsule selection.
...
- Current active milestone: none. `goal-plan-detail-header-timeline-alignment` is closed after implementation commit `7798f058ae1b272e8eca6dd8a31cba4490c8faa1` and hosted Governance CI #65 PASS. Phase 02 remains unselected, and next work requires explicit future capsule selection.
...
2. Active capsule document: none currently selected. Read a capsule document only after a future capsule is explicitly selected.
...
Next gate is explicit future capsule selection only. Phase 02 remains unselected. No Firebase setup, `flutterfire configure`, dependency installation, build, init, deploy, backend work, native work, or additional Flutter implementation is authorized without separate explicit approval.

## Operational TODO / Active Capsule

Active capsule: none. Next work requires explicit future capsule selection.
...
4. Treat the Goal Plan Detail, Weekly Workout Detail, Expert Plan List, Expert Plan Detail, and Goal Plan Detail header/timeline alignment capsules as closed unless a future explicit task reopens one of them.
...
9. `goal-plan-detail-header-timeline-alignment` closure preserves static sample daily rows as preview-only display data; they must not create enrollment, persistence, onboarding mutation, trusted current-week calculation, or backend-owned plan progress behavior.
