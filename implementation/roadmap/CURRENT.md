...
- Current active milestone: none selected. Latest reconciled local Run Tracking milestones are M4-A Local Run Tracking Engine complete at `c3ee282 fix(run): harden local tracking engine`, M4-B Foreground GPS Permission + Provider complete at `adad6f4 feat(run): add foreground GPS provider`, and M4-B permission guidance complete at `f9fe044 fix(run): improve foreground location permission guidance` after manual/emulator QA passed for the permission guidance path. Next recommended work is M4-C Map Follow Runner UI plan-only. Phase 02 remains unselected.

Run Tracking M4-A/M4-B reconciliation:
...
- Raw GPS samples remain local-only.
...
- Still not implemented: M4-C map follow-runner UI, route trace backend upload, shared route auto-generation, background tracking, production Firebase, XP/streak/level formula, and leaderboard aggregation.
- Next recommended work: M4-C Map Follow Runner UI plan-only.
...
Next gate is explicit next capsule selection or push/remote verification decision for the local implementation commits recorded before this roadmap reconciliation. Recommended next work is M4-C Map Follow Runner UI plan-only. No Phase 02 selection, production Firebase setup, `flutterfire configure`, dependency installation, build, init, deploy, native work, route trace backend upload, shared route auto-generation, background tracking, real XP/streak/level formula, leaderboard aggregation, or additional Flutter implementation is authorized without separate explicit approval.
