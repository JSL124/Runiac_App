...
- Current active capsule: `implementation/roadmap/capsules/leaderboard-map-first-landing-shell.md`
...
3. Active capsule document: `implementation/roadmap/capsules/leaderboard-map-first-landing-shell.md`
...
- Superseded prior capsule: `implementation/roadmap/capsules/leaderboard-static-motivation-hierarchy-polish.md` was superseded before implementation due to refined map-first UX direction.
- Type: Flutter static frontend-only Leaderboard landing shell capsule
- Completion commit target: `feat(mobile): add leaderboard map landing shell`
- Chain: A0_ORCH -> A9_TRACE -> A5_WIRE -> A10_FLUTTER_IMPL -> A6_REVIEW -> A12_QA_TEST -> A8_OUTPUT_CHECKER
- Allowed future implementation files: `implementation/mobile/runiac_app/lib/features/leaderboard/presentation/leaderboard_tab.dart`; `implementation/mobile/runiac_app/lib/features/shell/runiac_shell.dart` only for hiding/removing the top AppBar when the active tab is Leaderboard; `implementation/mobile/runiac_app/test/widget_test.dart` only if stable visible text expectations need updates.
- Required boundary: map-first landing shell only; shell change limited to Leaderboard-only AppBar suppression; no shell navigation redesign, bottom navigation changes, Home/Maps/Run/You AppBar behavior changes unless unavoidable and explicitly reported, global Scaffold refactor, route or navigation architecture changes, unrelated shell styling, help modal behavior, region tap behavior, region preview bottom sheet, bottom sheet visible by default, internal ranking preview, Phase 02 selection, Firebase, Auth, Firestore, Cloud Functions, FCM, GPS/current-location permission, real location state, real map SDK, real map tiles, real leaderboard data, leaderboard aggregation, XP/streak/level/rank/leaderboard score/weekly XP/monthly XP mutation, subscription privilege state, expert plan publication state, fake users, fake ranks, fake XP, fake scores, fake streaks, fake levels, Home, Maps, Run, You/Profile content changes, dependencies, native platform changes, scaffold, build, init, deploy, or unrelated screen changes.
...
- Current implementation state: no implementation has started for the new map-first capsule. Dirty Flutter files from an earlier uncommitted Leaderboard attempt remain unapproved context only and must not be staged, committed, reset, stashed, or cleaned as part of this routing state.
- Stop state: selected for implementation-approved static Leaderboard map-first landing shell only; do not select Phase 02, help modal behavior, or region bottom sheet behavior.
