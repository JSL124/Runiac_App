...
- Current active capsule in this isolated worktree: `implementation/roadmap/capsules/home-you-state-stability.md`. The user explicitly routed and then authorized implementation of this Home/You state-stability package as isolated work on branch `codex/home-you-state-stability`; the package is implemented and at `Ready for manual commit after user review`. The shared-tree cadence capsule remains separate and must not be modified by this isolated package.
...
- Current active milestone in this isolated worktree: `home-you-state-stability`. Phase 02 remains unselected.
- Newly routed backed Friends MVP on 2026-07-13 Asia/Singapore: `implementation/roadmap/capsules/friends-backend-mvp.md` is an explicitly delegated integrated Backend Guarded Lane capsule for authenticated nickname/profile identity callables, atomic request/friend/block transitions, rate/cooldown limits, Unicode migration, Rules/indexes, emulator tests, and Luna's listed Firebase/callable Friends/Profile Flutter paths. It supersedes the historical static Friends shell/refinement behavior at runtime: Suggested is removed and Friends / Search / Requests / Blocked use backed owner lists and callables. Historical capsules remain unchanged; Challenge remains out of scope. A later explicit user authorization permitted the scoped production deploy and commit: `runiac-fypp` now reports all 40 expected Functions ACTIVE, the Friends indexes present, and the nickname callable reachable with authentication enforced. Luna's Flutter verification remains separately owned.
...
Current capsule package in this isolated worktree: `implementation/roadmap/capsules/home-you-state-stability.md`. This package now authorizes the scoped Home/You/shell implementation, focused tests, capsule/CURRENT updates, and diff-hygiene allowlist entry recorded in the capsule. Firebase/backend, native files, dependencies, cadence files, Activity Feedback paths, and unrelated roadmap capsules remain forbidden. `implementation/roadmap/capsules/activity-history-feed-upload.md` is concurrent and separate. `implementation/roadmap/capsules/feed-friends-emulator-backend.md` remains a prior package at `Ready for user screen QA` and `Ready for manual commit`; `implementation/roadmap/capsules/adaptive-character-guidance.md` remains open and byte-for-byte unchanged. Existing Leaderboard, unrelated design/test, and debug-journal dirt remains unrelated, unstaged, and untouched.
