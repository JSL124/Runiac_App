...
- Most recent locally closed implementation milestone: M4-C2 Mapbox Run Map Surface closed at `4ac294c feat(run): add Mapbox run map surface`; roadmap reconciliation for this closure is pending commit.
- Current status: Phase 01 governance CI closed; Artifact Inventory Schema persistence completed; Repository Workflow Record capsule closed; Flutter scaffold baseline present at `implementation/mobile/runiac_app/`; multiple static Flutter UI capsules are closed through Run Hold-to-End static interaction at `027c960`; Run Tracking backend/repository/emulator milestones M2, M3, and M5 are complete locally through `ff345e3`; M4-A/M4-B foreground run tracking milestones are complete locally through `f9fe044`; M4-C1 Local Map Follow Runner UI is complete locally at `b5124dd feat(run): add local map follow UI`; M4-C2 Mapbox Run Map Surface is complete locally at `4ac294c feat(run): add Mapbox run map surface`; M4-F GPS Quality Diagnostics is implemented locally in the current worktree with blocker-fix validation passed and physical iPhone QA still pending. FlutterFire is allowed only for emulator-only `completeRun` wiring guarded by `RUNIAC_FIREBASE_EMULATOR=true`; production Firebase remains forbidden.
- Current state: Scaffold-baseline governance state with the Flutter mobile UI split into a feature-first-lite source structure, a narrow emulator-only Run completion backend path, a local Run Tracking engine, a foreground GPS permission/provider path, a Mapbox-backed run map surface gated by `MAPBOX_PUBLIC_ACCESS_TOKEN`, and local scalar GPS quality diagnostics for Run Tracking. Phase 02 remains unselected. The Run completion flow now has a repository seam, async hardening, emulator-only FlutterFire bootstrap, anonymous Auth emulator sign-in, and a `completeRun` callable adapter for `asia-southeast1` when `RUNIAC_FIREBASE_EMULATOR=true`; the static repository remains the default fallback when that flag is absent or false. Missing-token placeholder fallback remains available, Android no-token fallback QA passed, token-backed Mapbox demo readiness is pending real demo-token QA, and the Mapbox network/telemetry boundary is documented. Raw route/GPS samples remain local-only, M4-F diagnostics remain scalar/local-only, `completeRun` remains summary-only, physical iPhone QA for M4-F remains pending, route trace backend upload is not done yet, shared route auto-generation is not done yet, GPS trace persistence is not done yet, background tracking is not done yet, Auto Pause and Moving Time remain future M4-G scope, real XP/streak/level formula is not done yet, and leaderboard aggregation is not done yet.
...
- M4-F physical iPhone QA remains pending and must not be treated as complete.
...
Next gate is explicit token-backed Mapbox QA with a real demo public token, explicit next capsule selection, or push/remote verification decision for the local implementation commits recorded before this roadmap reconciliation. M4-C2 Mapbox Run Map Surface is complete locally at `4ac294c`, but token-backed demo readiness remains pending real-token QA. No Phase 02 selection, production Firebase setup, `flutterfire configure`, build, init, deploy, route trace backend upload, GPS trace persistence, shared route auto-generation, background tracking, real XP/streak/level formula, leaderboard aggregation, or additional Flutter implementation is authorized without separate explicit approval.
