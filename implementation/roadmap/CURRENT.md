...
- Current active capsule: `implementation/roadmap/capsules/running-activity-history-user-link.md`. This implementation-approved capsule connects Running Activity History to authenticated owner-scoped `runSummaries` reads, preserves static fallback and current-session overlay behavior, and keeps `completeRun` as the trusted write path. It does not approve Firestore rules/index deploy, Cloud Functions production deploy, `firebase init`, `flutterfire configure`, secrets/config, route trace upload, GPS sample persistence, XP/streak/level/rank/leaderboard formula, leaderboard aggregation, premium entitlement logic, expert-plan publication logic, Phase 02, backend-owned client mutation, or SwiftPM staging. The previous `implementation/roadmap/capsules/firestore-base-bootstrap-seam.md` remains the closed Firestore SDK/bootstrap seam capsule-of-record.
...
Active capsule: `implementation/roadmap/capsules/profile-onboarding-edit-profile.md`. The previous `implementation/roadmap/capsules/profile-persistence-rules-contract.md` capsule has been superseded locally by this expanded capsule for signup personal profile collection, Edit profile, and onboarding retake. The previous `implementation/roadmap/capsules/firestore-base-bootstrap-seam.md` capsule is closed after the Firestore SDK/bootstrap seam sequence. The previously active `implementation/roadmap/capsules/complete-run-cloud-functions-emulator-skeleton.md` capsule is closed after the broader Run Tracking M2/M3/M5 sequence.
