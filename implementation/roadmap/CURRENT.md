...
- Current active capsule: `implementation/roadmap/capsules/complete-run-cloud-functions-emulator-skeleton.md`. This explicitly routes emulator-only Backend Guarded Lane work for the first Cloud Functions `completeRun` skeleton; it does not select Phase 02, authorize production Firebase, deploy, FlutterFire client wiring, secrets, `.firebaserc`, real XP formulas, leaderboard aggregation, subscription privilege logic, expert/admin privilege logic, or client-owned mutation of backend-owned values.
...
- Current state: Scaffold-baseline governance state with the static Flutter mobile UI split into a feature-first-lite source structure; Phase 02 remains unselected; hosted GitHub Actions now runs `git diff --check`, `./tools/governance-ci/run-all-checks.sh`, Flutter SDK setup, `flutter pub get`, `flutter analyze --no-pub`, and `flutter test`; the Leaderboard information affordance still opens a centered static Tips popup/dialog; the Leaderboard league/division pill opens a separate static Leagues taxonomy popup; the region preview sheet remains draggable and static; Run launch displays a static full-screen map-style launch UI with top close/GPS/settings controls, shared circular tap feedback, static runner marker, and a floating Today's Plan / Start Run card; the Run Live Tracking state uses a compact static progress card with no internal Today's Plan / Running header, no Heart metric, no backend-owned value mutation, and a static Pause to Resume / End interaction with shared action-area width and matching action heights; paused End now requires press-and-hold with an internal progress/fill, keeps the visible label exactly `End`, normal tap does not end, early release resets, and completed hold remains an inert/no-op boundary; `you-plans-static-ui` is committed at `6624267`; the You page presentation-layer backend-read-model readiness refactor is committed and pushed at `acdbcff`; `goal-plan-detail-static-snapshot-shell` is closed after adding the static Goal Plan Detail screen from the You tab Plan section with bottom navigation preserved; `weekly-workout-detail-static-snapshot-shell` is closed after adding static Thu/Sat workout instruction previews, preserving non-clickable Rest/Completed rows, refining the preview-only schedule sheet, and validating local Flutter/governance checks plus Android emulator launch smoke; `expert-plan-list-static-snapshot-shell` is closed after adding the static You > Plans > Expert Plans list screen with visual-only filters, visual-only/no-op View Plan controls, `Coach reviewed` wording, `Healthy Running Starter Plan`, Runiac long stripe/search visual refinements, and preserved backend-owned boundaries; `expert-plan-detail-static-snapshot-shell` is closed after adding a static First 5K Plan Preview detail from the first Expert Plans card, keeping other cards no-op, using an independently expandable Plan Timeline with all weeks collapsed by default, removing `Who this is for` and `What you'll do`, rendering a disabled `Select This Plan` CTA, preserving backend-owned boundaries, and receiving hosted Governance CI #63 PASS for `bd5c4b2c118bf0ce3776747418a40aa0d8e14007`; `goal-plan-detail-header-timeline-alignment` is closed after refining the static Goal Plan Detail header/accent strip, visual-only week markers, horizontal dividers, dropdown daily rows, current-week highlight, removing visible weekly summary text, removing vertical marker connector lines, preserving backend-owned boundaries, and receiving hosted Governance CI #65 PASS for `7798f058ae1b272e8eca6dd8a31cba4490c8faa1`.
...
- Current active milestone: `complete-run-cloud-functions-emulator-skeleton` is selected as the active Backend Guarded Lane capsule. `flutter-static-ui-demo-config-extraction` is closed at `bdc1a47 refactor(mobile): extract static ui demo data`. Pre-reconciliation implementation state was clean. Latest local implementation commit before roadmap reconciliation: `6325e8d fix(maps): align shared route like cluster`. Local branch had advanced beyond `origin/main` by 7 implementation commits before this roadmap reconciliation. Phase 02 remains unselected.
...
2. Active capsule document: `implementation/roadmap/capsules/complete-run-cloud-functions-emulator-skeleton.md`.
...
- Do not run Flutter, Firebase, npm, build, test, deploy, scaffold, or init commands unless the current active capsule explicitly authorizes emulator-only validation commands. For `complete-run-cloud-functions-emulator-skeleton`, local `npm test --prefix functions`, `npm test --prefix tests/firebase-rules`, and governance shell checks are authorized for emulator-only validation only.
...
Next gate is explicit next capsule selection or push/remote verification decision for the local implementation commits recorded before this roadmap reconciliation; no stage, commit, push, Phase 02 selection, Firebase setup, `flutterfire configure`, dependency installation, build, init, deploy, backend work, native work, or additional Flutter implementation is authorized without separate explicit approval.

## Operational TODO / Active Capsule

Active capsule: `implementation/roadmap/capsules/complete-run-cloud-functions-emulator-skeleton.md`. This capsule is limited to emulator-only Cloud Functions `completeRun` skeleton work, backend-owned Firestore emulator writes, validation, idempotency, and tests. Previous capsule `implementation/roadmap/capsules/flutter-static-ui-demo-config-extraction.md` is closed after static Flutter frontend demo/display data and shared asset constant extraction. Before this roadmap reconciliation, the local implementation branch had advanced beyond `origin/main` by 7 implementation commits that include backend read-model contracts and Maps like-count UI polish.
...
3. Read the active capsule document before further action.
...
8. Do not add fake users/ranks/XP/scores/levels, plan completion calculations, completed-run calculations, remaining-run calculations, expert eligibility logic, Firebase/backend/GPS/native/dependency work, sharing integration, premium gating, subscription logic, or Phase 02 selection.
9. Do not select or implement another capsule, Phase 02 work, production Firebase, FlutterFire client wiring, GPS/native work, dependency work outside the active Functions/rules test scope, or any new product scope without separate explicit approval.
