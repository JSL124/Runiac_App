...
- Most recent completed capsule: `leaderboard-static-read-model-snapshot-readiness` implemented, validated, committed, pushed at `cff26ca refactor(leaderboard): isolate static display snapshots`, and hosted Governance CI passed.
...
- Current active milestone: Home / Maps static read-model snapshot readiness is implemented and validated as a small static frontend-only presentation refactor; Phase 02 remains unselected.
...
Next gate is post-push hosted Governance CI inspection for the `home-maps-static-read-model-snapshot-readiness` commit.
...
4. Keep implementation limited to Home and Maps presentation files plus relevant existing widget tests.
