...
- Current active capsule: none. The previous active capsule `implementation/roadmap/capsules/flutter-static-ui-demo-config-extraction.md` is closed after implementation, local validation, and commit `bdc1a47 refactor(mobile): extract static ui demo data`.
...
2. Active capsule document, if a future explicit task selects one. No active capsule is currently selected.
...
Active capsule: none. Previous capsule `implementation/roadmap/capsules/flutter-static-ui-demo-config-extraction.md` is closed after static Flutter frontend demo/display data and shared asset constant extraction. Before this roadmap reconciliation, the local implementation branch had advanced beyond `origin/main` by 7 implementation commits that include backend read-model contracts and Maps like-count UI polish; no backend, Firebase, native, dependency, navigation, run-tracking behavior, stage, commit, or push work is authorized by the closed capsule.
...
3. Read the active capsule document before any further action only after a future explicit task selects one.
4. Do not load future phase documents or route a new capsule unless explicitly requested.
...
9. Do not select or implement a next capsule, Phase 02 work, Firebase/backend/GPS/native/dependency work, or any new product scope without separate explicit approval.
