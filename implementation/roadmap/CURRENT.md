...
- Current active capsule: `implementation/roadmap/capsules/profile-persistence-rules-contract.md`. This is a narrow Firebase/FlutterFire profile persistence capsule for onboarding-time owner `userProfiles/{uid}` safe field writes plus matching local Firestore rules contract tests. It does not approve `users/{uid}` writes, Firestore rules/index deploy, broad feature Firestore reads/writes, production Cloud Functions deploy, OAuth, Phase 02, backend-owned client mutation, or SwiftPM staging. The previous `implementation/roadmap/capsules/firestore-base-bootstrap-seam.md` remains the closed Firestore SDK/bootstrap seam capsule-of-record.
...
10. Do not select or implement another capsule, Phase 02 work, Firestore persistence beyond onboarding owner `userProfiles/{uid}` safe fields, feature-level Firestore reads/writes, FlutterFire production config expansion beyond the existing Firebase config, GPS/native work, dependency work, or any new product scope without separate explicit approval.
