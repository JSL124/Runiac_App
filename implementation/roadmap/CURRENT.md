...
- Current active capsule: `implementation/roadmap/capsules/profile-onboarding-edit-profile.md`. This implementation-approved capsule expands the previous narrow profile persistence work to include the signup-time personal profile collection step, owner-scoped `userProfiles/{uid}` personal/onboarding profile persistence, Account > Manage > Edit profile, and onboarding retake from Edit profile. It does not approve `users/{uid}` writes, Firestore rules/index deploy, Cloud Functions deploy, OAuth, email update, route/activity/GPS/plan document persistence, subscription/role/progression/leaderboard/premium/expert-plan persistence, Phase 02, backend-owned client mutation, or SwiftPM staging. The previous `implementation/roadmap/capsules/firestore-base-bootstrap-seam.md` remains the closed Firestore SDK/bootstrap seam capsule-of-record.
