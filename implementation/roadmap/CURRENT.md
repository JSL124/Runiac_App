...
- Current active capsule: `implementation/roadmap/capsules/cadence-capture-reliability-recovery.md`. The user explicitly routed this production reliability fix on 2026-07-12 Asia/Singapore after physical-device cadence remained unavailable. It permits native phone-motion cadence recovery, strict Flutter event/lifecycle propagation, the shared 40–240 spm Dart/Functions contract, persistence regression validation, and focused native/Flutter/Functions QA without fabricated cadence. Concurrent Activity History recovery work remains separate and must not be modified beyond cadence persistence regression checks.
...
- Current status: Cadence capture reliability recovery is `In progress` after physical-device cadence remained unavailable. Activity History durable preview/feed recovery is concurrent and separate; it is not the active capsule and must remain untouched except for cadence persistence regression validation. The Activity Summary Agent Feedback implementation remains committed separately; prior Feed/Friends and unrelated work remain untouched.
...
- Current active milestone: `cadence-capture-reliability-recovery`. Phase 02 remains unselected.
...
Current capsule package: `implementation/roadmap/capsules/cadence-capture-reliability-recovery.md`. It authorizes only truthful native cadence recovery, strict Flutter propagation/diagnostics/lifecycle bounds, the shared 40–240 spm trusted validation contract, persistence regression checks, and focused native/Flutter/Functions evidence. `implementation/roadmap/capsules/activity-history-feed-upload.md` is concurrent and separate. `implementation/roadmap/capsules/feed-friends-emulator-backend.md` remains a prior package at `Ready for user screen QA` and `Ready for manual commit`; `implementation/roadmap/capsules/adaptive-character-guidance.md` remains open and byte-for-byte unchanged. Existing Leaderboard, unrelated design/test, and debug-journal dirt remains unrelated, unstaged, and untouched.
