...
- Production `users` state observed on 2026-07-20: six documents, all `userRole: "runner"` except `m3wD49fw3bamXhP4opmkwxkPIdj1`, which already carries `userRole: "platformAdmin"` on a real account. The `platformAdmin` document has no `accountStatus` field because a console role change merge-writes only `userRole` and `updatedAt`; every consumer defaults an absent value to `active`, so this is consistent rather than broken. Note the collection is populated, which contradicts an earlier working assumption that production had no `users` documents and would need a provisioning capsule before the console could be pointed at production.
- Provenance of that state, from a read-only sweep of production `adminAuditLogs` (24 entries): the admin console has been operating against **production, not the emulator, since at least 2026-07-19T18:06Z** — fourteen `feedback-triage` and two `feedback-close` entries at 18:06–18:14Z, then five `user.backfill.create-users-doc` entries at 18:25Z (actor `jason04334@gmail.com`, from `website/scripts/backfill-missing-users-docs.mjs`), which is where the six `users` documents came from, then `user.progression.level.set` at 18:27Z and `user.backfill.fix-level-xp-consistency` at 18:40Z. The console production cutover is therefore already history, not a future step. Every entry carries actor and before/after, so the audit trail itself behaved as designed.
- End-to-end expiry evidence is now pending rather than unobtainable. A real admin action at `2026-07-20T08:00:52Z` set `users/RtEOc6ujVKWtOAzBTwBVfgqoGLD2` from `basic`/null to `premium` with `subscriptionExpiresAt: 2026-07-21T23:59:59.999Z` (reason "Test"), stored as a Firestore `Timestamp` with `subscriptionSource: "admin-override"` — exactly the shape the contract requires, on real data rather than a synthetic document.
- Expected first downgrade: **2026-07-23 03:00 Asia/Singapore**, not 07-22. The expiry instant is `2026-07-21T23:59:59.999Z`, which is 2026-07-22 07:59:59 SGT, so the 07-22 03:00 sweep still runs before it lapses and only the 07-23 sweep selects it. This UTC-end-of-day storage is the console's own convention (`setUserSubscription()` parses `${date}T23:59:59.999Z`), so an expiry always lapses on the sweep after the one the calendar date suggests.
- Outstanding: confirm at that sweep that `subscriptionStatus` flips to `basic`, `subscriptionExpiresAt` is cleared, `subscriptionSource: "system-expiry"` is stamped, and an `adminAuditLogs` entry appears with actor `system` and action `user.subscription.expire`. Leave the granted premium in place until then; reverting it manually destroys the only pending production evidence. Do not manufacture evidence by writing a synthetic lapsed document into production `users`.
