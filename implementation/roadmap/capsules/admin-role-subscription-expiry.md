...
- `isPremiumSubscription(data, nowMs)` (`functions/src/progression/progressionAuditHelpers.ts`) treats a `premium` status whose `subscriptionExpiresAt` has passed as not premium for in-request XP/leaderboard evaluation. It tolerates a `Timestamp`, an ISO string, or a millis number; an unparseable value falls back to "no expiry". The required `nowMs` parameter is threaded from the existing call sites rather than read inside the helper, keeping it pure.
...
- New suites (`roles`, `progressionAuditHelpers`, `subscriptionExpiry`): 14/14 pass. Covers canonical and legacy role strings; no-expiry, future-expiry, past-expiry and boundary instants; `Timestamp` / ISO / millis forms; unparseable-value fallback; and a sweep that downgrades exactly the lapsed document, leaves future-expiry, lifetime-premium and already-basic documents untouched, and writes the audit entries.
- Regression over the `nowMs` blast radius (`completeRun`, `completeCoolDown`, `monthlyLeaderboard`, `monthlyLeaderboardWriter`, `progressionCalculator`): 112/112 pass.
