...
Status: Ready for commit after user-owned final visual acceptance.

## Goal

Make each Home stage-map background feel deliberately filled by seven larger stage stones, alternate one `<` chevron then one `>` chevron with centre-aligned seam endpoints, scale the guide character with the larger stones, use the supplied Blue idle animation only while the guide is stationary, and keep the first background's lowest weekday label clear of the bottom navigation.
...
- Use the supplied Blue runner GIF only for the stationary Home guide while preserving direction-specific PNG sprites during walking.
...
- No character-asset changes beyond registering the supplied Blue stationary idle GIF.
...
- Focused asset-selection tests prove the GIF is stationary-Blue-only and walking retains direction-specific PNG sprites.
- `flutter analyze --no-pub`, focused Flutter tests, `git diff --check`, and roadmap routing pass.
- Final visual acceptance is owned by the user, who explicitly asked Codex not to run a simulator. The supplied first-page screenshot is the spacing reference.

## Done When

- [x] Seven enlarged stones fill each background in a readable zigzag.
- [x] The guide character scales with the stones.
- [x] The supplied Blue idle GIF is restricted to the stationary Home guide state.
...
- [ ] User confirms final visual acceptance on their simulator.
