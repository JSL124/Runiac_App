...
| `check-canonical-root.sh` | Confirm active governance commands are launched from the canonical logical repository root. | Initial logical `PWD`, optional runner invocation path, physical `pwd -P`, and `git rev-parse --show-toplevel` | `/Users/leejinseo/Desktop/FYP_Runiac` only | Logical root is not `/Users/leejinseo/Documents/FYP_Runiac`, runner invocation is not through the Documents symlink path, Git root resolves to `/Users/leejinseo/Desktop/FYP_Runiac`, and physical working directory is inside the Desktop root. | Logical root or runner invocation is the Documents symlink path, Git root is not the Desktop root, or physical working directory is outside the Desktop root. |
| `check-diff-hygiene.sh` | Confirm staged or working diff is reviewable before commit. | `git status --short`, `git diff --check`, optional path-filtered `git diff --stat` | Task-approved files only; broad mobile allowlisting does not silently approve iOS/SwiftPM native artifacts. | No whitespace errors, no unrelated modified files, no unrelated iOS/SwiftPM native artifacts, and no forbidden generated/source/config files in the diff. | Whitespace errors, unrelated paths, unrelated iOS/SwiftPM native artifacts such as SwiftPM `Package.resolved`, `Podfile.lock`, or Xcode project drift, or forbidden implementation/config artifacts present. |
