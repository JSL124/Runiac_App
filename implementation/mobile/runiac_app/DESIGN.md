...
- **Structure**: `RuniacBackHeader` titled `Friends`, a compact 4-segment `YouSegmentedControl` (`Friends` / `Search` / `Suggested` / `Requests`), and white rows with radius 18 and a `border` outline. Every row leads with the compact 42px `RuniacLevelProfileBadge` and a w800 `textPrimary` display name. `Friends` rows stop there; `Search` and `Suggested` reserve a 44px trailing action target.
- **Per-tab states**: `Friends` lists accepted friends or an empty state. `Search` shows a hint until a query is typed, then case-insensitive name matches with a `No runners found` empty state. `Search` and `Suggested` show the supplied Add icon for available runners; tapping it changes only that runner to the supplied noninteractive Pending icon for the screen session. `Requests` preserves its existing invitation copy, filled `Accept` control, outlined `Decline` control, and empty state once none remain.
- **Session-local actions**: Add/Pending and Accept/Decline are local `setState` display state only, shared between Search and Suggested where the same runner appears, reset on screen re-entry, and never persisted.
- **Accessibility**: Add announces `Add <name>` and Pending announces `Pending <name>`; Pending has no action. Accept/Decline continue to announce `Accept <name>` / `Decline <name>` as buttons, the search field announces `Search runners`, and decorative badges/icons are excluded so the adjacent name remains the single readable identity.
