...
// ---------------------------------------------------------------------------
// Local reference + defensive read helpers
// ---------------------------------------------------------------------------
...
function grantRef(firestore: Firestore, challengeId: string, uid: string): DocumentReference {
  return firestore.collection("challengeRewardGrants").doc(`${challengeId}_${uid}`);
}
function badgeRef(firestore: Firestore, uid: string, tierId: string): DocumentReference {
  return firestore.doc(`users/${uid}/challengeBadges/${tierId}`);
}
function historyRef(firestore: Firestore, uid: string, challengeId: string): DocumentReference {
...
// ---------------------------------------------------------------------------
// Started-instance load (single read pass — all reads precede any write)
// ---------------------------------------------------------------------------
...
async function loadInstanceWithRoster(
...
function releaseSlotIfHeldHere(
  transaction: Transaction,
...
// Personalized history snapshot. `terminalReason` is omitted when the instance
// has not reached a terminal outcome yet (a leaver's mid-flight history doc).
function buildHistoryDoc(args: {
  ...
  return {
    ...
    role: readString(args.participantData, "role") === "owner" ? "owner" : "member",
    ...
    startedAt: Timestamp.fromMillis(timestampToMillis(args.instanceData["startsAt"])),
