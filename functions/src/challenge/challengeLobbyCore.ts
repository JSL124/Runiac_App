...
// ---------------------------------------------------------------------------
// Callable request shape (mirrors completeRun's minimal contract)
// ---------------------------------------------------------------------------
...
function instanceRef(firestore: Firestore, challengeId: string): DocumentReference {
  return firestore.collection("challengeInstances").doc(challengeId);
}
function participantRef(firestore: Firestore, challengeId: string, uid: string): DocumentReference {
  return instanceRef(firestore, challengeId).collection("participants").doc(uid);
}
function participantsQuery(firestore: Firestore, challengeId: string): Query {
  return instanceRef(firestore, challengeId).collection("participants");
}
function invitationRef(firestore: Firestore, inviteId: string): DocumentReference {
  return firestore.collection("challengeInvitations").doc(inviteId);
}
function invitationsForChallengeQuery(firestore: Firestore, challengeId: string): Query {
  return firestore.collection("challengeInvitations").where("challengeId", "==", challengeId);
}
function slotRef(firestore: Firestore, uid: string): DocumentReference {
  return firestore.collection("challengeSlots").doc(uid);
}
function profileRef(firestore: Firestore, uid: string): DocumentReference {
  return firestore.collection("userProfiles").doc(uid);
}
function friendRef(firestore: Firestore, ownerUid: string, otherUid: string): DocumentReference {
  return firestore.doc(`users/${ownerUid}/friends/${otherUid}`);
}
function blockRef(firestore: Firestore, ownerUid: string, otherUid: string): DocumentReference {
  return firestore.doc(`users/${ownerUid}/blockedUsers/${otherUid}`);
}

// Deterministic invitation id so duplicate invites collapse to one doc and a
// recipient responds by a stable, guessable-free id.
function invitationId(challengeId: string, recipientUid: string): string {
  return `${challengeId}__${recipientUid}`;
}

// ---------------------------------------------------------------------------
// Argument parsing (stable INVALID_ARGUMENT / UNKNOWN_TIER)
// ---------------------------------------------------------------------------
...
function requireAuthUid(request: CallableRequest): string {
  ...
  if (uid === undefined || uid.length === 0) throw challengeError("UNAUTHENTICATED");
  return uid;
}

function requireTierId(data: unknown): ChallengeTierId {
...
function requireChallengeId(data: unknown): string {
  const record = asRecord(data);
...
function requireInviteId(data: unknown): string {
  const record = asRecord(data);
...
// Parse + dedupe the invitee uid list. Empty lists and non-string entries are
// invalid arguments.
function requireInviteeUids(data: unknown): readonly string[] {
  const record = asRecord(data);
  const uids = record["uids"];
  if (!Array.isArray(uids) || uids.length === 0) throw challengeError("INVALID_ARGUMENT");
  const seen = new Set<string>();
  for (const value of uids) {
    if (typeof value !== "string" || value.length === 0) throw challengeError("INVALID_ARGUMENT");
    seen.add(value);
  }
  return [...seen];
}

function requireResponse(data: unknown): "accept" | "decline" {
  const record = asRecord(data);
...
// ---------------------------------------------------------------------------
// Runtime doc readers (defensive — Firestore is schemaless at rest)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Relationship read (same collections the feed reads; reuse feed decision)
// ---------------------------------------------------------------------------

async function readReciprocalRelationship(
  transaction: Transaction,
  firestore: Firestore,
  viewerUid: string,
  authorUid: string,
...
// ---------------------------------------------------------------------------
// Serialization (participant-safe read models)
// ---------------------------------------------------------------------------
...
function serializeInstance(challengeId: string, data: DocumentData): ChallengeInstanceView {
  ...
  return {
    ...
    scheduledEndsAtMs: scheduledEndsAt === undefined ? null : timestampToMillis(scheduledEndsAt),
    terminalReason: typeof terminalReason === "string" ? terminalReason : null,
  };
}

function serializeParticipant(data: DocumentData): ChallengeParticipantView {
  return {
...
function sortedParticipantViews(snapshot: QuerySnapshot): readonly ChallengeParticipantView[] {
